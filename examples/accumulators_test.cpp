...
// main.hpp
//
//  Taken and modified from boost example:
//
...
template <typename Range> void output_range(Range const &rng) {
  bool first = true;
  for (const auto d : rng) {
    if (!first)
      std::cout << ", ";
    std::cout << d;
    first = false;
...
void example1() {
  accumulator_set<
      double, stats<tag::min, tag::mean(immediate), tag::sum, tag::moment<2>>>
      acc;

  std::array<double, 4> data = {0., 1., -1., 3.14159};
  ...
  std::cout << "  min"
               "(acc)        = "
            << (min)(acc) << std::endl; // Extra quotes are to prevent
  ...
  std::cout << "  moment<2>(acc)  = " << accumulators::moment<2>(acc)
            << std::endl;
}
...
void example2() {
  ...
  std::cout << "  tail            = ";
  output_range(tail(acc));
  std::cout << "  tail_variate    = ";
  output_range(tail_variate(acc));
  std::cout << std::endl;
...
void example3() {
  ...
  // Weighted calculation with an external weight accumulator
  {
    ...
    std::cout << "  mean(acc)       = " << mean(acc, weights = weight_acc)
              << std::endl;
  }
}
